...
class BaseOptionsMenu extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		if(rpcTitle == null) rpcTitle = 'S*x Menu';
		...
		{
			optionText = new Alphabet(220, 260, optionsArray[i].name, false);
			...
			/* boring stuck colors lol
			var len = optionText.letters.length;
			for (i in 0...len) {
				var letter = optionText.letters[i];
				if (i < len * 0.25)
					letter.setColorTransform(1, 1, 1, 1, 255, 255, 153);
				else if (i < len * 0.5)
					letter.setColorTransform(1, 1, 1, 1, 255, 0, 0);
				else if (i < len * 0.75)
					letter.setColorTransform(1, 1, 1, 1, 139, 69, 19);
				else
					letter.setColorTransform(1, 1, 1, 1, 50, 205, 50);
			}
			*/
	...
	override function update(elapsed:Float)
	{
		...
		for (optionText in grpOptions.members) {
			for (i in 0...optionText.letters.length) {
				var scolori:Int = Std.int((i + gayshit) % hcolors.length); // selected color INDEX
				var scolor:Array<Int> = hcolors[scolori];

				optionText.letters[i].setColorTransform(1, 1, 1, 1, scolor[0], scolor[1], scolor[2]);
			}
		}

		for (valueText in grpTexts.members) {
			for (i in 0...valueText.letters.length) {
				var scolori:Int = Std.int(((i + 3) + gayshit) % hcolors.length); // selected color INDEX
				var scolor:Array<Int> = hcolors[scolori];

				valueText.letters[i].setColorTransform(1, 1, 1, 1, scolor[0], scolor[1], scolor[2]);
			}
		}
