...
class PlayState extends MusicBeatState
{
	...
	var statedead:Bool = false;
	public var boyfriendMap:Map<String, Character> = new Map<String, Character>();
	public var dadMap:Map<String, Character> = new Map<String, Character>();
	public var gfMap:Map<String, Character> = new Map<String, Character>();

	public var Fucked:Bool = false;
	var RS:Int; //Repeated Slash
	var RST:Bool = false;
	public var Spin:Bool = false;
	var Way:String;
	var WayFV:String; //Favourite video
	public var Switch:Bool = false;
	public var CA:Float = 0; //Current Angle
	public var s1:Null<Int> = null;
	public var s2:Null<Int> = null;
	public var Limit:Float = 0;
	var wu:Float; //WIndup More like Miwu haha..
	var VideoTrigger:Bool = false;
	public var canDodge:Bool = false;
	public var Dodged:Bool = false;
	public var Flipped:Bool = false;
	var HT:FlxTimer; //Hold timer
	var NT:FlxTimer; //Now timer
	var slash:FlxTimer; //Slash timer
	public var freeze:Bool = false;
	public var Lol:FlxVideoSprite;
	public var dodgeTime:Float = 0;
	public var Video:Array<String> = CoolUtil.coolTextFile(Paths.txt("secretstuff"));
	public var dodgeLimit:Float = 1;
	var Datime:Float;
	var warningText:FlxSprite;
	var DB2:Float;
	var DB2B64:haxe.Int64;
	var LOV:haxe.Int64;
	...
	public var bsop:Array<FlxPoint> = [];
	public var dsop:Array<FlxPoint> = [];
	var OP:Array<FlxPoint> = [];
	var TIS:Float = 0;
	...
	public static var HOC:Bool = false; //Head of Coder
	...
	//extra shit
	var JCR:FlxTextFormatMarkerPair = new FlxTextFormatMarkerPair(new FlxTextFormat(0x005DFF), "!"); //john corruption red
	var JCB:FlxTextFormatMarkerPair = new FlxTextFormatMarkerPair(new FlxTextFormat(0xffffcfcf), "?"); //john corruption black
	var lhealth:Float = -1;
	override public function create()
	{
		...
		if (dadGroup.x != null && boyfriendGroup.x != null) {
			OP = [
				new FlxPoint(boyfriendGroup.x, boyfriendGroup.y),
				new FlxPoint(dadGroup.x, dadGroup.y)
			];
		}
		else {
			OP = [
				new FlxPoint(BF_X, BF_Y),
				new FlxPoint(DAD_X, DAD_Y)
			];
		}
		*/

		warningText = new FlxSprite();
		warningText.frames = Paths.getSparrowAtlas('c00lwarning');
		warningText.animation.addByPrefix("hold", 'hold', 11, true);
		warningText.animation.addByPrefix("now", 'now', 32, true);
		//warningText.color = FlxColor.BLUE; // set the color to cyan
		warningText.scale.set(0.5, 0.5);
		warningText.updateHitbox();
		//warningText.setFormat("VCR OSD Mono", 32, FlxColor.BLUE, CENTER);
		warningText.screenCenter();
		add(warningText);
		warningText.visible = false;

		var HMV:Int = Video.length;

		TCO = FlxG.random.int(0, HMV - 1); //THE CHOSEN ONE Or TCO
		...
		WayFV = 'memes/$FV';
		/*
		Lol = new FlxVideoSprite(OP[1].x, OP[1].y);
		Lol.load(Paths.video(Way));
		if (ClientPrefs.data.SM) Lol.volumeAdjust = 0;
		//Lol.screenCenter(XY);
		*/
		...
		switch (curStage)
		{
			...
			case 'crossroads': new Crossroads(); //Week DOE
			case 'deadroads': new Deadroads(); // Week DOE - mar18
		}
		...
		switch (SONG.player1) {
			case "boy":
				GameOverSubstate.characterName = "boy1-dead";
			default:
		...
		//Lol = new FlxVideoSprite(OP[1].x, OP[1].y);
		Lol = HotEngine.catchlol;
		Lol.x = OP[1].x;
		Lol.y = OP[1].y;
		if (ClientPrefs.data.SM) Lol.volumeAdjust = 0;
		LOV = Int64.fromFloat(10e3); //Limit of VIdeo ;-;
		Lol.load(Paths.video(Way));
		add(Lol);
		Lol.visible = false;

		if (Lol != null) {
			Lol.bitmap.onEndReached.removeAll();
			statedead = true;
		}

		Lol.bitmap.onPlaying.add(() -> {

			DB2B64 = Int64.div(Lol.bitmap.duration, Int64.ofInt(2));

			if (Int64.compare(Lol.bitmap.duration, LOV) < 0) {
				//trace("shorty");
				Lol.bitmap.onEndReached.add(End);
			}
			else {
				Lol.bitmap.onTimeChanged.add(int64 -> {
					if (Int64.compare(Lol.bitmap.time, DB2B64) >= 0)
						End();
				});
			}
		});

		DB2 = cast(DB2B64, Float); //Duration by 2 more like DPS haha... GET OU-
		...
		//healthText = new FlxText(339.5, 622.8, 104, 'health:', 16); //debug is the GOAT
		healthText = new FlxText(0, 0, 104, 'health:', 16);
		switch [ClientPrefs.data.middleScroll, ClientPrefs.data.downScroll] { //switch better than if statements fr fr
			case [true, false]:
				healthText.y = 158.458598726115;
			case [false, true]:
				healthText.y = 615;
			case [true, true]:
				healthText.y = 551.541401273885;
			case [false, false]:
				healthText.y = 90;
		}
		switch (StageData.getStageFile(PlayState.curStage).directory) {
			case "weekdoe":
				if (curStage == 'deadroads') {
					healthText.color = 0xf8f296;
					healthText.applyMarkup('hea?lth:?!${health * 100}!', [JCB, JCR]);
				}
				else healthText.color = 0xf8f296;
			case "c00lweek":
				healthText.color = 0xfd1919;
		}
		healthText.visible = (ClientPrefs.data.SH ? true : false);
		healthText.screenCenter(X);
		healthText.height = 24;
	...
	public function startCountdown()
	{
		...
		if(ret != LuaUtils.Function_Stop) {
			...
			for (i in 0...playerStrums.length) {
				...
				bsop.push(new FlxPoint(playerStrums.members[i].x, 0));
			}
			for (i in 0...opponentStrums.length) {
				setOnScripts('defaultOpponentStrumX' + i, opponentStrums.members[i].x);
				setOnScripts('defaultOpponentStrumY' + i, opponentStrums.members[i].y);
				dsop.push(new FlxPoint(opponentStrums.members[i].x, 0));
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			/*backup number 1
			var assetshiet:Null<String> = null;

			switch(StageData.getStageFile(curStage).directory) {
				case 'weekdoe':
					StrumNote.texture = 'noteSkins/Not'
				case 'c00lweek':
					assetshiet = 'noteSkins/Tag';
				default:
					assetshiet = null;
			}
			...
			if (assetshiet != null) {
				for (strum in strumLineNotes)
					strum.texture = assetshiet;

				for (note in unspawnNotes) {
					note.texture = assetshiet;
					note.rgbShader.enabled = true;
					note.defaultRGB(); // reapply lane colors cuz why not
				}
			}
			
			backup shit lol number 2
			if (StrumNote.TerminalMap.exists(curStage)) {
				var asset:String = StrumNote.TerminalMap.get(curStage);
	...
	function End() {
		if (statedead == false || this.active == false) return;
		health = 0;
	}
	...
	override public function update(elapsed:Float)
	{
		...
		healthBar.leftToRight = (Flipped ? true : false);

		healthText.text = 'health:${health * 100}';

		if (health != lhealth && curStage == 'deadroads') {
			healthText.color = 0xf8f296;
			healthText.applyMarkup('hea?lth:?!${health * 100}!', [JCB, JCR]);
			lhealth = health;
		}
		...
		for (i in 0...playerStrums.length) {
			...
			playerStrums.members[i].x = (Flipped ? dsop[i].x : bsop[i].x);

			if (Switch) {
				s1 = FlxG.random.int(0, playerStrums.length - 1);
				s2 = FlxG.random.int(0, playerStrums.length - 1);

				if (s1 == s2) s1 = FlxG.random.int(0, playerStrums.length - 1);
			}

			if (s1 != null && s2 != null) {
				playerStrums.members[s1].x = (Flipped ? dsop[s2].x : bsop[s2].x);
				playerStrums.members[s2].x = (Flipped ? dsop[s1].x : bsop[s1].x);
			}
			var ratr:Float = FlxG.random.int(10, 270) / 0.004; //Rotation amount to random
			var RR:Float = ratr * elapsed; //Rotation Rate Duhh ;-;

			if (Spin) {
				playerStrums.members[i].angle += RR;
				CA += RR;
			}
			else if (!Spin)
				playerStrums.members[i].angle = CA;
			//trace(playerStrums.members[i].angle);
			//trace(CA);
		}
		for (i in 0...opponentStrums.length) {
			opponentStrums.members[i].x = (Flipped ? bsop[i].x : dsop[i].x);
		}

		if (boyfriend.curCharacter == 'boy')
			boyfriend.flipX = (Flipped ? false : true);
		else
			boyfriend.flipX = (Flipped ? true : false);
		...
		if (Flipped) {
			...
			if (dadGroup.x != DAD_X && boyfriendGroup.x != BF_X) {
				dadGroup.x = DAD_X;
		...
		if (Fucked) {
			var TDrain:Float = 0.2; //Total Drain
			var Time:Float = 3; //Time takes to drain
			var DR:Float = (TDrain / Time) * elapsed; //drain RATE

			health = Math.max(0.005, health - DR);
		...
		var LolChancer = FlxG.random.float(0, 99.9);

		function CheckLol(COD:Float = 5) { //COD = Chance of DEAD
			if (HOC == true) COD = 100;
			if (LolChancer < COD) {
				Lol.visible = true;
				if (HT != null) HT.cancel();
				if (NT != null) NT.cancel();
		...
		if (canDodge) {
			...
			if (controls.justPressed('dodge') && !Dodged)
			{
				canDodge = false;
				...
				if (dodgeTime >= Datime && dodgeTime <= dodgeLimit) {
					if (dad.animOffsets.exists("slash"))
						dad.playAnim("slash", true);
					else
						trace("No slash found");
				...
				if (dodgeTime < Datime)
				{
					...
					if (!CheckLol()) {
						...
						health = Math.max(health - 0.26, 0.07);
					}
				}
			
			}

			if (dodgeTime >= dodgeLimit)
			{
				canDodge = false;
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Slash':
				...
				RST = true;
				wu = 0.6;
				Datime = dodgeLimit - wu; //It must be perfect :p

				trace("GOTCHA BITCH");

				function DeadBySlash() {
					if (HT != null) HT.cancel();
					if (NT != null) NT.cancel();
					canDodge = true;
					Dodged = false;
					dodgeTime = 0; //Reset the time 2 dodge
					warningText.animation.play('hold');
					warningText.visible = true;
					HT = new FlxTimer().start(Datime, function(tmr:FlxTimer) {
						warningText.animation.play('now');
						NT = new FlxTimer().start((wu), function(tmr:FlxTimer) {
							warningText.visible = false;
						});
					});
				}
				...
				if (RS > 1) {
					slash = new FlxTimer().start(dodgeLimit + 1, timer -> DeadBySlash(), RS - 1);
				}
				else if (RS == 0) slash = new FlxTimer().start(dodgeLimit + 1, timer -> DeadBySlash(), 0); //Loop for eternity lol
			...
			case 'Spin':
				...
				var Loop:Int = (flValue2 != null) ? Std.int(flValue2) : 12;

				new FlxTimer().start(flValue1, function(Timer:FlxTimer) {
					Spin = !Spin;
					//trace(Spin);
				}, Loop);
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		{
			if (note.noteType == 'Corruption Note') {
				Fucked = true;
			}
	...
	override function destroy() {
		...
		if (Lol.exists == true) {
			statedead = false;
			Lol.bitmap.onEndReached.remove(End);
			remove(Lol, false);
		}
