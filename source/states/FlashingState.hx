...
class FlashingState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var warnText:FlxText = new FlxText(0, 0, FlxG.width,
			...
			Ts Mod may contain some copyrighted things (for example meme sounds)\n
			so to save your dih/buss from getting copyrighted\n
			Do you wish to enable Streamer/Youtuber Mode to remove copyright?");
		warnText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER);
		warnText.screenCenter(Y);
		texts.add(warnText);

		final keys = ["Hell yea", "Nah Ik my risks"];
		...
		texts.members[1].x = -240;
		texts.members[2].x = 208;
