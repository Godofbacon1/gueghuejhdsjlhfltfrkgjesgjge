...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Testing Stuff", null);
		#end
		...
		var bgStory:FlxSprite = new FlxSprite(100, 2).loadGraphic(Paths.getSharedPath("images/storyBG.png"));
		bgStory.antialiasing = ClientPrefs.data.antialiasing;
		FlxG.state.insert(0, bgStory);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			if(!isLocked || !weekFile.hiddenUntilUnlocked)
			{
				...
				weekThing.ID = num;
				weekThing.targetX = num * 500;
				weekThing.x = weekThing.targetX;
				weekThing.visible = (num == 0);
				itemTargetX += Math.max(weekThing.height, 110) + 10;
				...
				if (grpWeekText.members.length > 0)
					leftY = grpWeekText.members[0].y + 10;
				else
					leftY = 0.0;
			}
		}
		...
		leftArrow = new FlxSprite(850, 490);
		...
		sprDifficulty = new FlxSprite(560, leftArrow.y);
	...
	//var leftY:Float = grpWeekText.members[0].y + 10;
	...
	override function update(elapsed:Float)
	{
		...
		if (Section == "weeks")
			rightArrow.x = leftX + 411;
		else if (Section == "diff")
			rightArrow.x = leftX + sprDifficulty.graphic.width + 70;
			//rightArrow.x = leftX + 376;
		...
		if (!movedBack && !selectedWeek)
		{
			...
			if (Section == "weeks") {
				leftX = grpWeekText.members[curWeek].x - 50;
				//trace(leftX);
				leftY = grpWeekText.members[curWeek].y;
			}
			else if (Section == "diff") {
				leftX = sprDifficulty.x - 60;
				leftY = sprDifficulty.y - 20;
			}
		...
		{
			item.x = FlxMath.lerp(item.targetX - offX + 72.5, item.x, Math.exp(-elapsed * 10.2));
			item.y = 180;
			//trace(item.x);
			item.visible = (item.ID == curWeek);
		}
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		if(sprDifficulty.graphic != newImage)
		{
			...
			sprDifficulty.x = 490 - sprDifficulty.height - 118;
			//sprDifficulty.x += (308 - sprDifficulty.width) / 3; //X = 60 + (308 - sprDifficulty.width) / 3
			sprDifficulty.alpha = 0;
			sprDifficulty.y = 490 - sprDifficulty.height - 118;
			//trace(sprDifficulty.x);
			//trace(sprDifficulty.y);
			//trace(leftArrow.y);300

			FlxTween.cancelTweensOf(sprDifficulty);
			//FlxTween.tween(sprDifficulty, {y: sprDifficulty.y + 30, alpha: 1}, 0.07);
			var tweenval = (sprDifficulty.graphic.width > 300 ? 95 : 145);
			FlxTween.tween(sprDifficulty, {x: tweenval, alpha: 1}, 0.07);
		}
