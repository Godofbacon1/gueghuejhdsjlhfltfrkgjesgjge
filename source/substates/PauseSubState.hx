...
class PauseSubState extends MusicBeatSubstate
{
	...
	function regenMenu():Void {
		...
		for (num => str in menuItems) {
			var item = new Alphabet(45, 80, Language.getPhrase('pause_$str', str), true);
			...
			item.scaleX = 0.85;
			item.scaleY = 0.85;
			item.x = (item.targetY > 4 ? 675 : 45);
			item.y = ((item.targetY % 5) * item.distancePerItem.y) + item.startPosition.y;
			if (item.width > 474.3) {
				item.scaleX = 474.3 / item.width;
			}
